...
local ClientTab = Window:Tab({Title = "Client", Icon = "user"}) do
    ...
    ClientTab:Section({Title = "Client Stuff"})
...
ClientTab:Dropdown({
    ...
    List = {"Teleportation", "SetSpawnPoint"},
    Value = config.autorespawnType,
    Callback = function(choice)
        ...
        if config.autorespawnEnabled then
            ...
            if config.autorespawnConnections then
                for i, connection in pairs(config.autorespawnConnections) do
                    connection:Disconnect()
                end
            end
            config.autorespawnConnections = {}
            config.autorespawnDeathPosition = nil
            config.autorespawnEnabled = true
        end
    end
})
...
ClientTab:Toggle({
    ...
    Value = config.autorespawnEnabled,
    Callback = function(state)
        ...
        local function onCharacterAdded(character)
            if config.autorespawnType == "Teleportation" then
                if config.autorespawnDeathPosition then
                    local root = character:WaitForChild("HumanoidRootPart", 5)
                    if root then
                        task.wait(0.1)
                        root.CFrame = config.autorespawnDeathPosition
                        config.autorespawnDeathPosition = nil
            ...
            elseif config.autorespawnType == "SetSpawnPoint" then
                if config.SSEnabled and config.SpawnLocation then
                    local root = character:WaitForChild("HumanoidRootPart", 5)
                    if root then
                        root.CFrame = config.SpawnLocation
                    end
                end
            end
            ...
            if humanoid and rootPart then
                config.autorespawnConnections.died = humanoid.Died:Connect(function()
                    ...
                    elseif config.autorespawnType == "SetSpawnPoint" then
                        setSpawnLocation(rootPart.CFrame)
                        respawn(player)
                    end
                end)
            end
        ...
        config.autorespawnConnections.characterAdded = player.CharacterAdded:Connect(onCharacterAdded)
        
        safeNotify({
            Title = "AutoRespawn",
            Content = "Enabled - Mode: " .. config.autorespawnType,
            Audio = "rbxassetid://17208361335",
            Length = 1,
            Image = "rbxassetid://4483362458",
            BarColor = Color3.fromRGB(0, 255, 0)
...
-- BotTab/BTab
local BotTab = Window:Tab({Title = "Bot", Icon = "monitor"}) do
    BotTab:Section({Title = "Bot Master"})
    BotTab:Toggle({
        Title = "Auto Attack",
    ...
    BotTab:Section({Title = "Bot Settings"})
...
BotTab:Toggle({
    ...
    BotTab:Dropdown({
        ...
        List = {"tool:Activate()", "Leftclick"},
        Value = "tool:Activate()",
        Callback = function(choice)
            config.PrimaryAction = choice
            print("Primary Action set to:", choice)
        end
    })
...
-- MiscTab
Window:Line()
local MiscTab = Window:Tab({Title = "Miscellaneous", Icon = "settings"}) do
